...
`serve` 启动后打开 `http://127.0.0.1:8765/`,页面上按三步走:

1. **导入数据** —— 两个入口任选:
   - 「业务描述」:点「下载描述模板」,把企业情况、业务流程、数据情况、目标约束按提纲
     写成文字(有数字就写数字),粘贴或上传 `.txt`,由 S2 抽成结构化画像。
     这一条需要 `.env` 里配好模型端点。
   - 「画像 JSON」:点「下载 JSON 模板」拿到与引擎 Schema **完全一致**的骨架
     (字段错一个就导不进去),填完粘贴或上传;校验通过才入库,失败会指到具体字段。
2. **画像库** —— 导入的画像与内置示例列在一起。点进去看缺口、流程节点、数据源、计算参数;
   有源描述的画像还能「补充信息后重新抽取」,不用从头再填一遍。
3. **运行报告** —— 选模板撰写(秒级)或大模型撰写,跑完直接进报告页,
   可导出 Markdown 或打印成 PDF。

页面是服务端渲染的自包含 HTML,不依赖前端构建工具;模板下载不需要模型,
没有配 `.env` 也能先把数据填起来。导入的画像落在 `data/profiles/uploads/`,
描述原文落在 `data/profiles/descriptions/uploads/`(两者都已 gitignore)。
